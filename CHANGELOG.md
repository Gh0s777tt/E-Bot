...
- `[#676]` 🗂️ **Kokpit statusu funkcji** (Discovery B2, top-5) — `/modules` (Centrum sterowania) zmienione z listy przełączników w kokpit: kafelki-liczniki **Wszystkie / Aktywne / Wymaga konfiguracji / Wyłączone** (klikalny filtr) + kropka statusu przy każdym module. „Wymaga konfiguracji" = moduł włączony, ale config nie ma ustawień poza flagą (świeżo przełączony, nie dokończony — dokładnie pain z audytu **P3**: „włączyłem, a nic się nie dzieje").
  ...
  - **Test:** [`moduleState.test.ts`](dashboard/lib/moduleState.test.ts) — `moduleConfigured` (bool zawsze OK; json bez zapisu / tylko flaga → needsConfig; własna ścieżka flagi; zły JSON). Kontekst: [`DISCOVERY_REPORT.md`](docs/archive/DISCOVERY_REPORT.md) (propozycja B2) + [szkic](docs/design/sketches/sketch-B2-kokpit-statusu.html).
...
- `[#675]` 💳 **Upsell w miejscu tarcia zamiast znikającego tekstu** (Discovery A2, fala 1/N; #1 priorytet biznesowy) — limit Free był widoczny dopiero PO kliknięciu Zapisz (403), jako tekst znikający po 4 s, bez licznika (problem P2). Nowy komponent [`UsageMeter`](dashboard/components/UsageMeter.tsx) pokazuje **proaktywnie** pasek „X / N · Free", a na/blisko limitu renderuje **klikalny** [`PremiumDialog`](dashboard/components/PremiumDialog.tsx) (→ Stripe Checkout) i wyłącza „Dodaj", nim user trafi w błąd.
  ...
  - **Test:** [`UsageMeter.test.ts`](dashboard/components/UsageMeter.test.ts) — progi `usageLevel` (ok/near/over, grandfathering, limit 0). Kontekst: [`DISCOVERY_REPORT.md`](docs/archive/DISCOVERY_REPORT.md) (problem P2, propozycja A2).
...
- `[#674]` 🩹 **Prawdziwy błąd zapisu zamiast generyku** (Discovery A1, fala 1/N) — panel pokazywał „Błąd zapisu" bez powodu: **46/47 formularzy** gubiło konkretny `{ error }` z API (limit planu / „kanał nie istnieje" / brak uprawnień). Wspólny helper [`saveConfig`](dashboard/lib/saveConfig.ts) wyciąga ten komunikat i podaje go do `SaveButton` (który już umiał go wyświetlić — pole `errorText` było „martwe"). **Zero nowych kluczy i18n** — komunikat jest dynamiczny z serwera (jak w istniejącym `ReactionRolesForm`).
  ...
  - **Kontekst:** wynika z audytu produktowego [`DISCOVERY_REPORT.md`](docs/archive/DISCOVERY_REPORT.md) (problem P1, propozycja A1 — #2 priorytet; + szkice A2/B2 w [`sketches/`](docs/design/sketches)).
...
- `[#673]` 🧹 **Remediacja samo-audytu** ([`AUDIT_REPORT.md`](docs/archive/AUDIT_REPORT.md)) — po przejściu read-only wykonano naprawy; **dwa ustalenia skorygowane** (błędy po stronie audytu), reszta domknięta:
...
- `[#618]` 🔒 **Fix false-negative anty-phishing** (defekt #1 z przeglądu QA, [`TEST_REPORT.md`](docs/archive/TEST_REPORT.md)). `hostOf` w [`contentScan.mts`](bot/src/lib/contentScan.mts) używał case-sensitive `url.startsWith('http')` → adres `HTTP://` / `HtTp://` (schemat URL jest case-insensitive wg RFC 3986) dostawał doklejone `http://` i hostname `"http"`, omijając WSZYSTKIE reguły hostowe `scanScam` (podrabiany Discord/Steam, IP, punycode, lista blokad). Poprawka jednoliniowa: `url.toLowerCase().startsWith('http')`. Test regresyjny ([`qa-findings.test.ts`](bot/src/qa-findings.test.ts)) — `HTTP://`/`HtTp://` wykrywane jak `http://`.
