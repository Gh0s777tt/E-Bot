    ...
    <div className="space-y-6">
      ...
      <PanelTabs
        tabs={[
          {
            id: 'activity',
            label: tp(lang, 'ui.statstab.activity'),
            panel: (
              <>
                {/* AI — 14 dni */}
                ...
                <section className="panel-glow rounded-2xl border border-line bg-card p-5">
                  <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-semibold tracking-wide">
                    ...
                    {tp(lang, 'ui.stats.activityHeading')}
                  </h2>
                  ...
                  <div className="mt-3 flex flex-wrap gap-4 text-sm">
                    <span>
                      ...
                      <strong className="text-accent">
                        {actTotals.messages.toLocaleString('pl-PL')}
                      </strong>
                    </span>
                    <span>
                    ...
                    <span>
                      ...
                      <strong className="text-accent">
                        {actTotals.voice.toLocaleString('pl-PL')}
                      </strong>
                    </span>
                  </div>
                  ...
                  {memEnough ? (
                    <>
                      <div className="mb-3 flex flex-wrap items-baseline gap-2">
                        ...
                        >
                          {memDelta >= 0 ? '+' : ''}
                          {memDelta.toLocaleString('pl-PL')} / {members.length}{' '}
                          {tp(lang, 'ui.home.sgDays')}
                        </span>
                      </div>
                      ...
                      <div className="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                          <div className="mb-1 text-xs text-muted">
                            {tp(lang, 'ui.home.boosts')}
                          </div>
                          <AreaChart values={boostsTrend} height={90} />
                        </div>
                        <div>
                          <div className="mb-1 text-xs text-muted">
                            {tp(lang, 'ui.home.channels')}
                          </div>
                    ...
                    <>
                      <div className="grid grid-cols-3 gap-3">
                        {(
                          ...
                          <div
                            key={k}
                            className="rounded-xl border border-line bg-elevated p-4 text-center"
                          >
          ...
          {
            id: 'community',
            label: tp(lang, 'ui.statstab.community'),
            panel: (
              <>
                <section className="panel-glow rounded-2xl border border-line bg-card p-5">
                ...
                <section className="panel-glow rounded-2xl border border-line bg-card p-5">
                  <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-semibold tracking-wide">
                    ...
                    {tp(lang, 'ui.stats.topActiveHeading')}
                  </h2>
                  {topUsers.length === 0 ? (
                        ...
                        <div key={u.user_id} className="flex items-center gap-3 text-sm">
                          ...
                          <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-elevated">
                            <div
                              className="h-full rounded-full bg-accent"
                              style={{
                                width: `${(u.messages / Math.max(1, topUsers[0].messages)) * 100}%`,
                              }}
                            />
                          </div>
          ...
          {
            id: 'other',
            label: tp(lang, 'ui.statstab.other'),
            panel: (
              <>
                {/* Tickety + biblioteka */}
                <div className="grid gap-4 lg:grid-cols-2">
                  <section className="panel-glow rounded-2xl border border-line bg-card p-5">
                    <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-semibold tracking-wide">
                      ...
                      {tp(lang, 'ui.stats.ticketsHeading')}
                    </h2>
                    <ul className="space-y-2 text-sm">
                      <li>
                        {tp(lang, 'ui.stats.tkOpen')}{' '}
                        <strong className="text-accent">{tk.open}</strong>
                      </li>
                      <li>
                        {tp(lang, 'ui.stats.tkClaimed')} <strong>{tk.claimed}</strong>
                      </li>
                      <li>
                        {tp(lang, 'ui.stats.tkClosed')}{' '}
                        <strong className="text-muted">{tk.closed}</strong>
                      </li>
                    </ul>
                  </section>
