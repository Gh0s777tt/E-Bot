...
-- ═══════════════════════════════════════════════════════════════════════════
-- GameVault + konfiguracja panel↔bot — SCALONE z dashboard/supabase/schema.sql.
-- Do teraz `games` i `settings` żyły WYŁĄCZNIE w tamtym pliku, wzmiankowanym tylko w
-- dashboard/README.md, a nagłówek TEGO pliku każe operatorowi uruchomić jedynie _ALL.sql.
-- Efekt: PostgREST 404 na obu tabelach — i KAŻDA ścieżka odczytu połyka to po cichu
-- (web/lib/db.ts → [], web/lib/settings.ts → DEFAULTS, dashboard/lib/data.ts → SQLite,
-- którego na Vercelu nie ma). Naprawa B-2/B-3 jest w kodzie poprawna, ale na bazie
-- postawionej wg dokumentacji byłaby martwa. Definicje WIERNE źródłu (nie zmieniamy
-- dashboard/supabase/schema.sql); pilnowane przez scripts/check-schema-sync.mjs, który
-- od teraz skanuje też dashboard/supabase/. Idempotentne jak reszta pliku.
-- ═══════════════════════════════════════════════════════════════════════════

-- 🎮 Biblioteka gier (kolektory Steam · PSN · GOG · IGDB → dashboard/scripts/seed-supabase.mts)
...
create index if not exists idx_games_playtime on games (playtime_min desc);

-- ⚙️ Konfiguracja panel ↔ bot (klucz/wartość; override per-serwer jako 'g:<gid>:<klucz>' —
-- dev_reset_guild() niżej kasuje właśnie ten prefiks, więc tabela MUSI istnieć w tym pliku).
...
-- `games` dostaje jawną politykę READ (tak jak w źródle): web/ czyta bibliotekę i może lecieć
-- na kluczu anon, gdy SUPABASE_SERVICE_ROLE_KEY nie jest ustawiony (fallback w web/lib/supabase.ts).
-- Bez tej polityki deny_client_access z sekcji Advisorów niżej (obejmuje tabele RLS BEZ polityk)
-- zamieniłby GameVault w pustą stronę. `settings` zostaje bez polityki = backend-only (service_role).
drop policy if exists "games read" on games;
create policy "games read" on games for select using (true);

-- (Opcjonalnie) Realtime dla `settings` — natychmiastowy sync bota po zmianie z panelu.
