...
import type { ClanRow } from '../lib/public';

const MEDALS = ['🥇', '🥈', '🥉'];

// Tablica rankingowa klanów (styl Dowództwa, bliźniaczo do LeaderboardBoard). Wartość = wspólny bank,
// pasek względem lidera rankingu; podpis = liczebność. Wiersz linkuje do profilu LIDERA (/p/u/[owner]).
export default function ClanBoard({
  ...
  const max = Math.max(1, ...rows.map((r) => r.bank));
  return (
