...
// (Biblioteka). Chunk 7: /appearance, /commands, /custom-commands, /integrations, /profile,
// /settings (grupa „Inne"). KOMPLET — wszystkie 37 stron przetłumaczone na 14 języków.
...
export const HOW_CONTENT_I18N: Partial<Record<PanelLocale, Record<string, HowEntry>>> = {
  en: {
    '/appearance': {
      does: 'Graphics look: theme, accent color and the styles of rank and profile cards generated by the bot.',
      why: 'You match the bot’s graphics to the server’s branding (colors, style).',
    },
    '/commands': {
      does: 'A full, always up-to-date list of the bot’s slash commands (pulled from Discord), grouped by category.',
      why: 'A cheat sheet for you and your moderators — what the bot can do and how to call it.',
    },
    '/custom-commands': {
      does: 'A no-code slash-command editor: your own commands with embeds, arguments, a role condition and actions (grant a role / give currency / XP).',
      why: 'You create features tailored to your server without programming — the panel registers them in Discord for you.',
      perms: [{ perm: 'Manage Roles', why: 'if the command grants/removes roles in its actions' }],
      tips: ['Registering a new command in Discord propagates within ~1 h.'],
    },
    '/integrations': {
      does: 'Keys and connections (Twitch, YouTube, AI, Supabase, Stripe…) and their status. This is where you enter missing API keys.',
      why: 'Features that need external services (live notifications, AI, database) come alive only after you enter the keys here.',
      tips: [
        'No key = the feature runs in a “dormant” mode and honestly says so — nothing breaks.',
        'After you add a key, the feature turns on by itself (the bot refreshes its config every ~30 s).',
      ],
    },
    '/profile': {
      does: 'Your card: level, economy, badges and activity history.',
      why: 'A view of your own achievements on the server in one place.',
    },
    '/settings': {
      does: 'Bot personalization: reply language, theme, panel access (who can configure) and config backup/restore.',
      why: 'The control center for everything — including who on your team can access the panel and at what level.',
      tips: [
        'Make a config backup before big changes — easy to roll back.',
        'You set the bot’s reply language independently of the panel language.',
      ],
    },
    '/eco': {
  ...
  de: {
    '/appearance': {
      does: 'Aussehen der Grafiken: Theme, Akzentfarbe sowie die Stile der Rang- und Profilkarten, die der Bot erzeugt.',
      why: 'Du passt die Grafiken des Bots an das Branding deines Servers an (Farben, Stil).',
    },
    '/commands': {
      does: 'Eine vollständige, stets aktuelle Liste der Slash-Befehle des Bots (direkt aus Discord abgerufen), nach Kategorien gegliedert.',
      why: 'Ein Spickzettel für dich und deine Moderatoren — was der Bot kann und wie man es aufruft.',
    },
    '/custom-commands': {
      does: 'Ein No-Code-Editor für Slash-Befehle: eigene Befehle mit embeds, Argumenten, einer Rollenbedingung und Aktionen (Rolle vergeben / Währung geben / XP).',
      why: 'Du erstellst Funktionen, die genau auf deinen Server zugeschnitten sind, ganz ohne Programmieren — das Panel registriert sie für dich in Discord.',
      perms: [
        {
          perm: 'Rollen verwalten',
          why: 'falls der Befehl in seinen Aktionen Rollen vergibt/entzieht',
        },
      ],
      tips: [
        'Die Registrierung eines neuen Befehls in Discord verbreitet sich innerhalb von ~1 h.',
      ],
    },
    '/integrations': {
      does: 'Schlüssel und Verbindungen (Twitch, YouTube, AI, Supabase, Stripe…) sowie deren Status. Hier trägst du fehlende API-Schlüssel ein.',
      why: 'Funktionen, die externe Dienste benötigen (Live-Benachrichtigungen, AI, Datenbank), erwachen erst zum Leben, nachdem du hier die Schlüssel eingetragen hast.',
      tips: [
        'Kein Schlüssel = die Funktion läuft im „Ruhemodus“ und sagt das ehrlich — nichts geht kaputt.',
        'Nachdem du einen Schlüssel hinzugefügt hast, schaltet sich die Funktion von selbst ein (der Bot aktualisiert seine Konfiguration alle ~30 s).',
      ],
    },
    '/profile': {
      does: 'Deine Karte: Level, Wirtschaft, Abzeichen und Aktivitätsverlauf.',
      why: 'Ein Überblick über deine eigenen Erfolge auf dem Server an einem Ort.',
    },
    '/settings': {
      does: 'Personalisierung des Bots: Antwortsprache, Theme, Panel-Zugriff (wer konfigurieren darf) und Sicherung/Wiederherstellung der Konfiguration.',
      why: 'Die Steuerzentrale für alles — einschließlich, wer aus deinem Team Zugang zum Panel hat und auf welcher Ebene.',
      tips: [
        'Erstelle vor großen Änderungen eine Sicherung der Konfiguration — so machst du sie leicht rückgängig.',
        'Die Antwortsprache des Bots stellst du unabhängig von der Sprache des Panels ein.',
      ],
    },
    '/eco': {
  ...
  es: {
    '/appearance': {
      does: 'Aspecto de los gráficos: tema, color de acento y los estilos de las tarjetas de rango y de perfil que genera el bot.',
      why: 'Ajustas los gráficos del bot a la identidad de tu servidor (colores, estilo).',
    },
    '/commands': {
      does: 'Una lista completa y siempre actualizada de los comandos slash del bot (obtenida de Discord), agrupada por categorías.',
      why: 'Una chuleta para ti y tus moderadores — qué puede hacer el bot y cómo invocarlo.',
    },
    '/custom-commands': {
      does: 'Un editor de comandos slash sin código: tus propios comandos con embeds, argumentos, una condición de rol y acciones (otorgar un rol / dar moneda / XP).',
      why: 'Creas funciones a medida de tu servidor sin programar — el panel las registra por ti en Discord.',
      perms: [{ perm: 'Gestionar roles', why: 'si el comando otorga/quita roles en sus acciones' }],
      tips: ['El registro de un nuevo comando en Discord se propaga en ~1 h.'],
    },
    '/integrations': {
      does: 'Claves y conexiones (Twitch, YouTube, AI, Supabase, Stripe…) y su estado. Aquí introduces las claves de API que falten.',
      why: 'Las funciones que necesitan servicios externos (notificaciones en directo, AI, base de datos) cobran vida solo después de que introduzcas aquí las claves.',
      tips: [
        'Sin clave = la función funciona en modo «inactivo» y lo indica con honestidad — nada se rompe.',
        'Tras añadir una clave, la función se activa sola (el bot actualiza su configuración cada ~30 s).',
      ],
    },
    '/profile': {
      does: 'Tu tarjeta: nivel, economía, insignias e historial de actividad.',
      why: 'Una vista de tus propios logros en el servidor en un solo lugar.',
    },
    '/settings': {
      does: 'Personalización del bot: idioma de las respuestas, tema, acceso al panel (quién puede configurar) y copia/restauración de la configuración.',
      why: 'El centro de control de todo — incluido quién de tu equipo tiene acceso al panel y con qué nivel.',
      tips: [
        'Haz una copia de la configuración antes de grandes cambios — la revertirás con facilidad.',
        'El idioma de las respuestas del bot se ajusta de forma independiente al idioma del panel.',
      ],
    },
    '/eco': {
  ...
  it: {
    '/appearance': {
      does: 'Aspetto della grafica: tema, colore d’accento e gli stili delle schede di rango e profilo generate dal bot.',
      why: 'Adatti la grafica del bot al branding del tuo server (colori, stile).',
    },
    '/commands': {
      does: 'Un elenco completo e sempre aggiornato dei comandi slash del bot (recuperato da Discord), suddiviso per categorie.',
      why: 'Un promemoria per te e i tuoi moderatori — cosa sa fare il bot e come richiamarlo.',
    },
    '/custom-commands': {
      does: 'Un editor di comandi slash senza codice: comandi personalizzati con embed, argomenti, una condizione di ruolo e azioni (assegna un ruolo / dai valuta / XP).',
      why: 'Crei funzioni su misura per il tuo server senza programmare — il pannello le registra per te in Discord.',
      perms: [
        { perm: 'Gestire i ruoli', why: 'se il comando assegna/rimuove ruoli nelle sue azioni' },
      ],
      tips: ['La registrazione di un nuovo comando in Discord si propaga entro ~1 h.'],
    },
    '/integrations': {
      does: 'Chiavi e connessioni (Twitch, YouTube, AI, Supabase, Stripe…) e il loro stato. Qui inserisci le chiavi API mancanti.',
      why: 'Le funzioni che richiedono servizi esterni (notifiche live, AI, database) prendono vita solo dopo che hai inserito qui le chiavi.',
      tips: [
        'Nessuna chiave = la funzione gira in modalità «sospesa» e lo segnala onestamente — non si rompe nulla.',
        'Dopo aver aggiunto una chiave, la funzione si attiva da sola (il bot aggiorna la sua configurazione ogni ~30 s).',
      ],
    },
    '/profile': {
      does: 'La tua scheda: livello, economia, badge e cronologia delle attività.',
      why: 'Una panoramica dei tuoi traguardi sul server in un unico posto.',
    },
    '/settings': {
      does: 'Personalizzazione del bot: lingua delle risposte, tema, accesso al pannello (chi può configurare) e backup/ripristino della configurazione.',
      why: 'Il centro di controllo per tutto — incluso chi del tuo team ha accesso al pannello e a quale livello.',
      tips: [
        'Fai un backup della configurazione prima di grandi modifiche — la ripristinerai facilmente.',
        'La lingua delle risposte del bot si imposta indipendentemente dalla lingua del pannello.',
      ],
    },
    '/eco': {
  ...
  fr: {
    '/appearance': {
      does: 'Apparence des visuels : thème, couleur d’accent et styles des cartes de rang et de profil générées par le bot.',
      why: 'Tu accordes les visuels du bot à l’identité de ton serveur (couleurs, style).',
    },
    '/commands': {
      does: 'Une liste complète et toujours à jour des commandes slash du bot (récupérée depuis Discord), regroupée par catégorie.',
      why: 'Un aide-mémoire pour toi et tes modérateurs — ce que le bot sait faire et comment l’appeler.',
    },
    '/custom-commands': {
      does: 'Un éditeur de commandes slash sans code : tes propres commandes avec embeds, arguments, une condition de rôle et des actions (attribuer un rôle / donner de la monnaie / XP).',
      why: 'Tu crées des fonctionnalités sur mesure pour ton serveur sans programmer — le panneau les enregistre pour toi dans Discord.',
      perms: [
        {
          perm: 'Gérer les rôles',
          why: 'si la commande attribue/retire des rôles dans ses actions',
        },
      ],
      tips: ['L’enregistrement d’une nouvelle commande dans Discord se propage en ~1 h.'],
    },
    '/integrations': {
      does: 'Clés et connexions (Twitch, YouTube, AI, Supabase, Stripe…) ainsi que leur statut. C’est ici que tu saisis les clés API manquantes.',
      why: 'Les fonctionnalités nécessitant des services externes (notifications live, AI, base de données) ne prennent vie qu’après avoir saisi les clés ici.',
      tips: [
        'Pas de clé = la fonctionnalité tourne en mode « en veille » et le signale honnêtement — rien ne casse.',
        'Une fois la clé ajoutée, la fonctionnalité s’active d’elle-même (le bot rafraîchit sa configuration toutes les ~30 s).',
      ],
    },
    '/profile': {
      does: 'Ta carte : niveau, économie, badges et historique d’activité.',
      why: 'Un aperçu de tes propres réalisations sur le serveur, au même endroit.',
    },
    '/settings': {
      does: 'Personnalisation du bot : langue des réponses, thème, accès au panneau (qui peut configurer) et sauvegarde/restauration de la configuration.',
      why: 'Le centre de contrôle de l’ensemble — y compris qui dans ton équipe peut accéder au panneau et à quel niveau.',
      tips: [
        'Fais une sauvegarde de la configuration avant de grands changements — facile à annuler.',
        'Tu règles la langue des réponses du bot indépendamment de la langue du panneau.',
      ],
    },
    '/eco': {
  ...
  pt: {
    '/appearance': {
      does: 'Aspeto dos visuais: tema, cor de destaque e os estilos dos cartões de rank e de perfil gerados pelo bot.',
      why: 'Adaptas os visuais do bot à identidade do teu servidor (cores, estilo).',
    },
    '/commands': {
      does: 'Uma lista completa e sempre atualizada dos comandos slash do bot (obtida do Discord), agrupada por categoria.',
      why: 'Um guia rápido para ti e para os teus moderadores — o que o bot sabe fazer e como o invocar.',
    },
    '/custom-commands': {
      does: 'Um editor de comandos slash sem programação: os teus próprios comandos com embeds, argumentos, uma condição de cargo e ações (atribuir um cargo / dar moeda / XP).',
      why: 'Crias funcionalidades feitas à medida do teu servidor sem programar — o painel regista-as por ti no Discord.',
      perms: [{ perm: 'Gerir cargos', why: 'se o comando atribui/remove cargos nas suas ações' }],
      tips: ['O registo de um novo comando no Discord propaga-se em ~1 h.'],
    },
    '/integrations': {
      does: 'Chaves e ligações (Twitch, YouTube, AI, Supabase, Stripe…) e o respetivo estado. É aqui que introduzes as chaves de API em falta.',
      why: 'As funcionalidades que precisam de serviços externos (notificações live, AI, base de dados) só ganham vida depois de introduzires aqui as chaves.',
      tips: [
        'Sem chave = a funcionalidade corre em modo “adormecido” e informa-o honestamente — nada se estraga.',
        'Depois de adicionares a chave, a funcionalidade ativa-se sozinha (o bot atualiza a configuração a cada ~30 s).',
      ],
    },
    '/profile': {
      does: 'O teu cartão: nível, economia, distintivos e histórico de atividade.',
      why: 'Uma vista das tuas próprias conquistas no servidor, num só lugar.',
    },
    '/settings': {
      does: 'Personalização do bot: idioma das respostas, tema, acesso ao painel (quem pode configurar) e cópia/restauro da configuração.',
      why: 'O centro de controlo de tudo — incluindo quem da tua equipa tem acesso ao painel e a que nível.',
      tips: [
        'Faz uma cópia da configuração antes de grandes alterações — é fácil reverter.',
        'Defines o idioma das respostas do bot de forma independente do idioma do painel.',
      ],
    },
    '/eco': {
  ...
  zh: {
    '/appearance': {
      does: '图形外观：机器人生成的等级卡和个人资料卡的主题、强调色及样式。',
      why: '让机器人的图形与服务器的品牌风格（颜色、样式）保持一致。',
    },
    '/commands': {
      does: '机器人斜杠命令的完整列表，始终保持最新（从 Discord 拉取），并按类别分组。',
      why: '为你和管理员准备的速查表——机器人能做什么，以及如何调用。',
    },
    '/custom-commands': {
      does: '无需编码的斜杠命令编辑器：创建带嵌入消息、参数、身份组条件和动作（授予身份组／发放货币／XP）的自定义命令。',
      why: '无需编程即可打造贴合你服务器的功能——面板会自动在 Discord 中注册它们。',
      perms: [{ perm: '管理身份组', why: '当命令在动作中授予／移除身份组时' }],
      tips: ['在 Discord 中注册新命令最多需 ~1 h 才能生效。'],
    },
    '/integrations': {
      does: '密钥与连接（Twitch、YouTube、AI、Supabase、Stripe……）及其状态。在此填入缺失的 API 密钥。',
      why: '需要外部服务的功能（开播通知、AI、数据库）只有在此处填入密钥后才会启用。',
      tips: [
        '缺少密钥＝功能进入「休眠」模式并如实告知——不会出任何故障。',
        '添加密钥后功能会自动启用（机器人每 ~30 s 刷新一次配置）。',
      ],
    },
    '/profile': {
      does: '你的卡片：等级、经济、徽章和活动记录。',
      why: '在一处查看你在服务器上的全部成就。',
    },
    '/settings': {
      does: '机器人个性化：回复语言、主题、面板访问权限（谁可以配置）以及配置的备份／恢复。',
      why: '整体的控制中心——包括团队中谁能访问面板以及拥有何种权限级别。',
      tips: ['进行重大更改前先备份配置——可轻松回滚。', '机器人的回复语言与面板语言可独立设置。'],
    },
    '/eco': {
  ...
  ko: {
    '/appearance': {
      does: '그래픽 외형: 봇이 생성하는 랭크 카드와 프로필 카드의 테마, 강조 색상 및 스타일.',
      why: '봇의 그래픽을 서버의 브랜딩(색상, 스타일)에 맞춥니다.',
    },
    '/commands': {
      does: '봇의 슬래시 명령어 전체 목록으로, 항상 최신 상태이며(Discord에서 가져옴) 카테고리별로 분류됩니다.',
      why: '나와 관리자를 위한 치트시트 — 봇이 무엇을 할 수 있고 어떻게 호출하는지 알려줍니다.',
    },
    '/custom-commands': {
      does: '코딩이 필요 없는 슬래시 명령어 편집기: 임베드, 인수, 역할 조건 및 동작(역할 부여／화폐 지급／XP)을 갖춘 나만의 명령어를 만듭니다.',
      why: '프로그래밍 없이 서버에 맞춘 기능을 만듭니다 — 패널이 알아서 Discord에 등록해 줍니다.',
      perms: [{ perm: '역할 관리', why: '명령어가 동작에서 역할을 부여／회수하는 경우' }],
      tips: ['Discord에 새 명령어를 등록하면 최대 ~1 h 안에 반영됩니다.'],
    },
    '/integrations': {
      does: '키와 연결(Twitch, YouTube, AI, Supabase, Stripe…) 및 그 상태. 여기서 누락된 API 키를 입력합니다.',
      why: '외부 서비스가 필요한 기능(라이브 알림, AI, 데이터베이스)은 여기에 키를 입력해야 비로소 작동합니다.',
      tips: [
        '키 없음＝기능이 「대기」 모드로 작동하며 이를 솔직하게 알려 줍니다 — 아무것도 망가지지 않습니다.',
        '키를 추가하면 기능이 저절로 켜집니다(봇이 ~30 s마다 설정을 새로고침합니다).',
      ],
    },
    '/profile': {
      does: '나의 카드: 레벨, 경제, 배지 및 활동 기록.',
      why: '서버에서 쌓은 나의 성과를 한곳에서 확인합니다.',
    },
    '/settings': {
      does: '봇 개인화: 응답 언어, 테마, 패널 접근 권한(누가 설정할 수 있는지) 및 설정 백업／복원.',
      why: '모든 것을 위한 컨트롤 센터 — 팀에서 누가 어떤 수준으로 패널에 접근할 수 있는지까지 포함합니다.',
      tips: [
        '큰 변경 전에 설정을 백업하세요 — 손쉽게 되돌릴 수 있습니다.',
        '봇의 응답 언어는 패널 언어와 별개로 설정합니다.',
      ],
    },
    '/eco': {
  ...
  ru: {
    '/appearance': {
      does: 'Вид графики: тема, акцентный цвет и стили карточек рангов и профиля, которые генерирует бот.',
      why: 'Ты подгоняешь графику бота под брендинг сервера (цвета, стиль).',
    },
    '/commands': {
      does: 'Полный, всегда актуальный список слэш-команд бота (загружается из Discord), сгруппированный по категориям.',
      why: 'Шпаргалка для тебя и твоих модераторов — что бот умеет и как это вызвать.',
    },
    '/custom-commands': {
      does: 'Редактор слэш-команд без кода: собственные команды с эмбедами, аргументами, условием по роли и действиями (выдать роль / дать валюту / XP).',
      why: 'Ты создаёшь функции под свой сервер без программирования — панель сама регистрирует их в Discord.',
      perms: [
        { perm: 'Управление ролями', why: 'если команда выдаёт/забирает роли в своих действиях' },
      ],
      tips: ['Регистрация новой команды в Discord распространяется до ~1 h.'],
    },
    '/integrations': {
      does: 'Ключи и подключения (Twitch, YouTube, AI, Supabase, Stripe…) и их статус. Здесь ты вписываешь недостающие API-ключи.',
      why: 'Функции, требующие внешних сервисов (уведомления о трансляциях, AI, база данных), оживают только после ввода ключей здесь.',
      tips: [
        'Нет ключа = функция работает в «спящем» режиме и честно об этом сообщает — ничего не ломается.',
        'После добавления ключа функция включается сама (бот обновляет конфиг каждые ~30 s).',
      ],
    },
    '/profile': {
      does: 'Твоя карточка: уровень, экономика, значки и история активности.',
      why: 'Обзор собственных достижений на сервере в одном месте.',
    },
    '/settings': {
      does: 'Персонализация бота: язык ответов, тема, доступ к панели (кто может настраивать) и резервная копия/восстановление конфигурации.',
      why: 'Центр управления всем — в том числе кто из твоей команды имеет доступ к панели и на каком уровне.',
      tips: [
        'Сделай резервную копию конфигурации перед большими изменениями — легко откатишься.',
        'Язык ответов бота ты задаёшь независимо от языка панели.',
      ],
    },
    '/eco': {
  ...
  uk: {
    '/appearance': {
      does: 'Вигляд графіки: тема, акцентний колір і стилі карток рангів та профілю, які генерує бот.',
      why: 'Ти підлаштовуєш графіку бота під брендинг сервера (кольори, стиль).',
    },
    '/commands': {
      does: 'Повний, завжди актуальний список слеш-команд бота (завантажується з Discord), згрупований за категоріями.',
      why: 'Шпаргалка для тебе і твоїх модераторів — що бот уміє і як це викликати.',
    },
    '/custom-commands': {
      does: 'Редактор слеш-команд без коду: власні команди з вбудованими повідомленнями, аргументами, умовою за роллю та діями (видати роль / дати валюту / XP).',
      why: 'Ти створюєш функції під свій сервер без програмування — панель сама реєструє їх у Discord.',
      perms: [{ perm: 'Керування ролями', why: 'якщо команда видає/забирає ролі у своїх діях' }],
      tips: ['Реєстрація нової команди в Discord поширюється до ~1 h.'],
    },
    '/integrations': {
      does: 'Ключі та підключення (Twitch, YouTube, AI, Supabase, Stripe…) і їхній статус. Тут ти вписуєш відсутні API-ключі.',
      why: 'Функції, що потребують зовнішніх сервісів (сповіщення про трансляції, AI, база даних), оживають лише після введення ключів тут.',
      tips: [
        'Немає ключа = функція працює в «сплячому» режимі й чесно про це повідомляє — нічого не ламається.',
        'Після додавання ключа функція вмикається сама (бот оновлює конфіг кожні ~30 s).',
      ],
    },
    '/profile': {
      does: 'Твоя картка: рівень, економіка, значки та історія активності.',
      why: 'Огляд власних здобутків на сервері в одному місці.',
    },
    '/settings': {
      does: 'Персоналізація бота: мова відповідей, тема, доступ до панелі (хто може налаштовувати) та резервна копія/відновлення конфігурації.',
      why: 'Центр керування всім — зокрема хто з твоєї команди має доступ до панелі та на якому рівні.',
      tips: [
        'Зроби резервну копію конфігурації перед великими змінами — легко відкотиш.',
        'Мову відповідей бота ти задаєш незалежно від мови панелі.',
      ],
    },
    '/eco': {
  ...
  ja: {
    '/appearance': {
      does: 'グラフィックの見た目：ボットが生成するランクカードとプロフィールカードのテーマ、アクセントカラー、スタイル。',
      why: 'ボットのグラフィックをサーバーのブランディング（色、スタイル）に合わせます。',
    },
    '/commands': {
      does: 'ボットのスラッシュコマンドの完全な一覧で、常に最新の状態を保ち（Discord から取得）、カテゴリ別に分類されています。',
      why: 'あなたとモデレーターのためのチートシート — ボットに何ができ、どう呼び出すかがわかります。',
    },
    '/custom-commands': {
      does: 'コード不要のスラッシュコマンドエディター：埋め込み、引数、ロール条件、アクション（ロール付与／通貨付与／XP）を備えた独自のコマンドを作成します。',
      why: 'プログラミングなしでサーバーに合わせた機能を作れます — パネルが Discord に自動で登録します。',
      perms: [{ perm: 'ロールの管理', why: 'コマンドがアクションでロールを付与／剥奪する場合' }],
      tips: ['Discord に新しいコマンドを登録すると、反映まで最大 ~1 h かかります。'],
    },
    '/integrations': {
      does: 'キーと接続（Twitch、YouTube、AI、Supabase、Stripe…）およびそのステータス。ここで不足している API キーを入力します。',
      why: '外部サービスを必要とする機能（ライブ通知、AI、データベース）は、ここにキーを入力して初めて動き出します。',
      tips: [
        'キーなし＝機能は「休止」モードで動作し、その旨を正直に知らせます — 何も壊れません。',
        'キーを追加すると機能は自動で有効になります（ボットは ~30 s ごとに設定を更新します）。',
      ],
    },
    '/profile': {
      does: 'あなたのカード：レベル、エコノミー、バッジ、アクティビティ履歴。',
      why: 'サーバーでの自分の実績を一か所で確認できます。',
    },
    '/settings': {
      does: 'ボットのカスタマイズ：応答言語、テーマ、パネルへのアクセス（誰が設定できるか）、設定のバックアップ／復元。',
      why: 'すべてを統括するコントロールセンター — チームの誰がどのレベルでパネルにアクセスできるかも含みます。',
      tips: [
        '大きな変更の前に設定をバックアップしましょう — 簡単に元に戻せます。',
        'ボットの応答言語はパネルの言語とは別に設定します。',
      ],
    },
    '/eco': {
  ...
  ar: {
    '/appearance': {
      does: 'مظهر الرسوميات: السمة ولون التمييز وأنماط بطاقات الرتب والملف الشخصي التي يُنشئها البوت.',
      why: 'تُطابق رسوميات البوت مع هوية السيرفر (الألوان، النمط).',
    },
    '/commands': {
      does: 'قائمة كاملة ومحدَّثة دائمًا بأوامر slash الخاصة بالبوت (تُجلب من Discord)، مُقسَّمة حسب الفئات.',
      why: 'ورقة مرجعية لك ولمشرفيك — ماذا يستطيع البوت أن يفعل وكيف تستدعيه.',
    },
    '/custom-commands': {
      does: 'محرّر أوامر slash بدون برمجة: أوامرك الخاصة مع تضمينات ووسائط وشرط الدور وإجراءات (منح دور / إعطاء عملة / XP).',
      why: 'تُنشئ ميزات مُفصَّلة على سيرفرك دون برمجة — واللوحة تُسجّلها في Discord نيابةً عنك.',
      perms: [{ perm: 'إدارة الأدوار', why: 'إذا كان الأمر يمنح/يسحب أدوارًا ضمن إجراءاته' }],
      tips: ['تسجيل أمر جديد في Discord ينتشر خلال ~1 h.'],
    },
    '/integrations': {
      does: 'المفاتيح والاتصالات (Twitch، YouTube، AI، Supabase، Stripe…) وحالتها. هنا تُدخل مفاتيح API الناقصة.',
      why: 'الميزات التي تحتاج خدمات خارجية (إشعارات البث المباشر، AI، قاعدة البيانات) لا تعمل إلا بعد إدخال المفاتيح هنا.',
      tips: [
        'لا مفتاح = الميزة تعمل في وضع «خامل» وتُخبرك بذلك بصراحة — لا شيء يتعطّل.',
        'بعد إضافة مفتاح، تُفعَّل الميزة من تلقاء نفسها (يُحدّث البوت إعداداته كل ~30 s).',
      ],
    },
    '/profile': {
      does: 'بطاقتك: المستوى، الاقتصاد، الأوسمة وسجلّ النشاط.',
      why: 'عرض لإنجازاتك على السيرفر في مكان واحد.',
    },
    '/settings': {
      does: 'تخصيص البوت: لغة الردود، السمة، الوصول إلى اللوحة (من يستطيع التهيئة) ونسخ/استعادة الإعدادات.',
      why: 'مركز التحكم بكل شيء — بما في ذلك مَن من فريقك يصل إلى اللوحة وبأي مستوى.',
      tips: [
        'اعمل نسخة احتياطية من الإعدادات قبل التغييرات الكبيرة — يسهُل التراجع.',
        'تضبط لغة ردود البوت بشكل مستقل عن لغة اللوحة.',
      ],
    },
    '/eco': {
  ...
  id: {
    '/appearance': {
      does: 'Tampilan grafis: tema, warna aksen, serta gaya kartu rank dan profil yang dibuat oleh bot.',
      why: 'Kamu menyelaraskan grafis bot dengan identitas servermu (warna, gaya).',
    },
    '/commands': {
      does: 'Daftar lengkap dan selalu terbarui dari perintah slash bot (diambil dari Discord), dikelompokkan menurut kategori.',
      why: 'Contekan untukmu dan para moderatormu — apa yang bisa dilakukan bot dan cara memanggilnya.',
    },
    '/custom-commands': {
      does: 'Editor perintah slash tanpa coding: perintah buatanmu sendiri dengan embed, argumen, syarat peran, dan aksi (berikan peran / beri mata uang / XP).',
      why: 'Kamu membuat fitur yang dirancang khusus untuk servermu tanpa pemrograman — panel mendaftarkannya untukmu di Discord.',
      perms: [{ perm: 'Kelola Peran', why: 'jika perintah memberi/mencabut peran dalam aksinya' }],
      tips: ['Pendaftaran perintah baru di Discord menyebar dalam ~1 h.'],
    },
    '/integrations': {
      does: 'Kunci dan koneksi (Twitch, YouTube, AI, Supabase, Stripe…) beserta statusnya. Di sinilah kamu memasukkan kunci API yang masih kurang.',
      why: 'Fitur yang membutuhkan layanan eksternal (notifikasi live, AI, basis data) baru hidup setelah kamu memasukkan kunci di sini.',
      tips: [
        'Tanpa kunci = fitur berjalan dalam mode “tidur” dan jujur memberitahukannya — tidak ada yang rusak.',
        'Setelah kamu menambahkan kunci, fitur menyala dengan sendirinya (bot menyegarkan konfigurasinya setiap ~30 s).',
      ],
    },
    '/profile': {
      does: 'Kartumu: level, ekonomi, lencana, dan riwayat aktivitas.',
      why: 'Pratinjau pencapaianmu sendiri di server dalam satu tempat.',
    },
    '/settings': {
      does: 'Personalisasi bot: bahasa balasan, tema, akses panel (siapa yang boleh mengonfigurasi), serta pencadangan/pemulihan konfigurasi.',
      why: 'Pusat kendali untuk segalanya — termasuk siapa di timmu yang boleh mengakses panel dan pada tingkat apa.',
      tips: [
        'Buat cadangan konfigurasi sebelum perubahan besar — mudah dikembalikan.',
        'Kamu mengatur bahasa balasan bot secara terpisah dari bahasa panel.',
      ],
    },
    '/eco': {
