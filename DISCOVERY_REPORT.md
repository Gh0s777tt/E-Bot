...
| ~~P3~~ | ⚠️ **WYCOFANE (fałszywy alarm)** — provisioning ISTNIEJE: panel→`setup_provision`→bot `provision.mts` tworzy kanały/role idempotentnie (`startProvision` w `index.mts:197`); komendy `/blueprint`,`/aiserver` też | weryfikacja #679 | Niuans: presety nie provisionują, blueprinty tak — drobne UX, nie brak funkcji |
| ~~P4~~ | ⚠️ **WYCOFANE (fałszywy alarm)** — rebrand JUŻ zrobiony w kodzie | weryfikacja #677: `community.ts:20` + `Footer.tsx:15` = „E‑Forge"; „GH0ST EMPIRE" w zrzutach = dane serwera właściciela / zrzuty sprzed #640 | GH0ST EMPIRE = świadoma nazwa ekosystemu (#572, `LICENSE`); `/economy` = realna integracja, nie błąd |
...
**A3. ~~Domknięcie rebrandingu + odczepienie `/economy`~~ — ⚠️ WYCOFANE (fałszywy alarm, weryfikacja #677)**
- **Sprostowanie:** rebrand jest **już zrobiony w kodzie** — [`dashboard/lib/community.ts:20`](dashboard/lib/community.ts) (seed powitania = „…na serwerze **E‑Forge**!") i [`dashboard/components/Footer.tsx:15`](dashboard/components/Footer.tsx) (stopka = „© 2026 E‑BOT · **E‑Forge**"). „GH0ST EMPIRE" w moich zrzutach to **dane własnego serwera właściciela** (jego serwer *nazywa się* GH0ST EMPIRE — więc jego skonfigurowane powitanie tak brzmi) albo zrzuty **sprzed** rebrandu #640.
- Marka „GH0ST EMPIRE" jest **świadomie zachowana** jako nazwa ekosystemu / podmiotu praw autorskich — decyzja z CHANGELOG **#572** („w domyślnych tekstach zostaje — to nazwa projektu, nie wyciek"), potwierdza `LICENSE`. `/economy` (GT) to **realna integracja** z portalem GHOST właściciela (`bot/src/empire/*`), nie błąd — gatowanie jej byłoby ingerencją w jego osobny tor.
- **Wniosek:** brak zmiany w kodzie. To kolejny fałszywy alarm oparty na zrzutach, nie na bieżącym kodzie (jak RLS 12→5 i „stray console" w audycie #673). **P4 należy uznać za nie‑ustalenie.**
...
**B1. ~~Prowadzona aktywacja, która realnie konfiguruje serwer~~ — ⚠️ WYCOFANE (fałszywy alarm, weryfikacja #679)**
- **Sprostowanie:** „kreator nie tworzy struktury" to **nieprawda** — provisioning istnieje i działa end-to-end. Panel (`Blueprints.tsx`, `ServerArchitect.tsx`) zapisuje plan do klucza `setup_provision`; **bot [`bot/src/setup/provision.mts`] (poll co 4 s, `startProvision` w `index.mts:197`) tworzy kategorie/kanały/role idempotentnie** (pomija istniejące po nazwie; wymaga ManageChannels/ManageRoles) i zwraca log. Komendy `/blueprint` i `/aiserver` tworzą strukturę wprost. `/setup` renderuje oba: Architekt + Blueprinty.
- **Skąd błąd:** agenci discovery patrzyli tylko na panel (zapis do DB) i przeoczyli konsumenta po stronie bota — **ten sam błąd co RLS 12→5, „stray console" i A3** (findingi z panelu/zrzutów przeceniają luki). Silnik jest też w **torze provisioningu właściciela** (`bot/src/setup/`), którego nie ruszam — budowa „generycznego B1" dublowałaby działający kod.
- **Ewentualna resztka (Niski):** *presety* (`lib/setup.ts` `apply()`) ustawiają tylko `enabled:true` bez provisioningu, a *blueprinty* robią jedno i drugie — user klikający preset (nie blueprint) może nie dostać struktury. To drobny niuans UX „preset vs blueprint", nie brak funkcji. **P3 = nie‑ustalenie co do rdzenia.**
...
3. ~~**B1**~~ — **WYCOFANE** (fałszywy alarm #679: provisioning już istnieje — `bot/src/setup/provision.mts` + Blueprinty/Architekt; budowa dublowałaby tor właściciela). Realne pozostałe: fale A1/A2 (mechaniczne) lub weryfikacja C1–C3.
4. ~~**A3**~~ — **WYCOFANE** (fałszywy alarm, #677: rebrand już zrobiony w kodzie; „GH0ST EMPIRE" to świadoma nazwa ekosystemu). Slot przejmuje **A1 fala 2** (prawdziwe błędy zapisu na kolejnych formularzach — mechaniczne, helper gotowy) lub **B1** (prowadzona aktywacja).
