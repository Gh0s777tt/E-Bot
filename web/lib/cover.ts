...
// Obraz przez lokalne proxy Next (działa też tam, gdzie przeglądarka nie ma dostępu do CDN).
export function proxied(u: string): string {
  if (u.startsWith('data:')) return u;
  return `/api/img?u=${encodeURIComponent(u)}`;
}

// Łańcuch fallbacków okładki: wybrana → kapsuła Steam → header Steam → placeholder.
