// Best-effort sliding-window rate-limit dla publicznych tras GameVault (/api/img, /api/settings).
// LUSTRO dashboard/lib/rateLimit.ts 1:1 — web/ celowo nie ma zależności współdzielonych z panelem
// (osobny projekt Next, minimalne `dependencies`), więc kopiujemy ~20 linii zamiast dokładać paczkę.
...
// (np. `img:<ip>` / `settings:<ip>`), by różne sinki nie współdzieliły kubełka.
