<!-- SYNC: v0.317.0 · #387 · 2026-06-20 — utrzymywane przez `pnpm docs:check` (NIE edytuj ręcznie wersji bez aktualizacji statusu) -->
<div align="center">
...
![Wersja](https://img.shields.io/badge/wersja-0.317.0-E50914?labelColor=0a0a0a)
...
- [ ] **Hardening po audycie całości (P0→P3)** — pełny **5-wymiarowy audyt** (bezpieczeństwo · zależności · kompletność · UX/wydajność · jakość/testy; werdykt: **0 podatności zależności, 0 TODO/stub, type-safety wzorowa**). **P0 w toku:** nagłówki bezpieczeństwa HTTP (CSP/HSTS/X-Frame/nosniff) w panelu + GameVault ✅ **v0.308.0** + anty-eskalacja `/roleperms`+`/rolecopy` ✅ **v0.308.0** + ujednolicenie uprawnień bota → Administrator ✅ **v0.309.0** + sekret admina (fail-closed) na `web/ /api/settings` ✅ **v0.309.0**; **bramka instance-admin na sekretach globalnych (Ko-fi + webhook-relay)** ✅ **v0.310.0** (reszta globalnych tras: ai_config/integrations/social_feeds/setup = kolejna partia) + **testy rdzenia bezpieczeństwa (20 vitest) + E2E Playwright w CI** ✅ **v0.311.0**. **Priorytety P0+P1 z audytu domknięte**; **„ogon" #2 — 4 globalne configi (integrations/ai-config/locale/presence) zabramkowane** ✅ **v0.312.0**; zostają trasy per-serwer do konwersji (setup/social_feeds/scheduled/creator) + **observability: błędy serwera → Sentry (onRequestError)** ✅ **v0.313.0** + **perf: edge-cache `/api/img`** ✅ **v0.314.0** + **UX/perf: GuildSwitcher `router.refresh` + `loading.tsx` serwerowy** ✅ **v0.315.0** + **a11y: prymityw `useFocusTrap` + dialog `MobileNav`** ✅ **v0.316.0** + **`CommandPalette` (dialog)** ✅ **v0.317.0** + dalej P2/P3 (next/image, kolejne overlaye, kontrast, monolity)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
<div align="center"><sub>Ostatnia aktualizacja: 2026‑06‑20 · v0.317.0 (#387) · powiązane: <a href="ROADMAP.md">ROADMAP</a> · <a href="../CHANGELOG.md">CHANGELOG</a> · weryfikacja sync: <code>pnpm docs:check</code></sub></div>
