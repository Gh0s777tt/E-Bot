...
![i18n panelu](https://img.shields.io/badge/i18n_panelu-🔄_14%2F~40-FFB020?labelColor=0a0a0a)
...
- [x] **Zrobione 14/~40 stron**: `/welcome` `/modules` `/leaderboard` `/logging` `/audit` `/modmail` `/birthdays` `/suggestions` `/appearance` `/wishlist` `/notifications` `/live` `/tickets` `/security`
- [ ] **Pozostało ~24 strony**: `/ai` `/applications` `/automations` `/commands` `/counters` `/creator` `/custom-commands` `/diagnostics` `/donations` `/eco` `/economy` `/engagement` `/gaming` `/integrations` `/levels` `/library` `/moderation` `/profile` `/responder` `/roles` `/scheduled` `/settings` `/setup` `/stats`
...
| **i18n treści + UI** | `0.202`–`0.224` | Samouczek, asystent, „Jak to działa?" 37/37, web GameVault (+RTL), **UI panelu (w toku, 14/~40)** | 🔄 |
...
- [🔄] **i18n UI panelu** — 14/~40 stron *(patrz „Bieżący tor" u góry)*

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
<div align="center"><sub>Ostatnia aktualizacja: 2026‑06‑18 · v0.224.0 (#294) · powiązane: <a href="ROADMAP.md">ROADMAP</a> · <a href="../CHANGELOG.md">CHANGELOG</a> · weryfikacja sync: <code>pnpm docs:check</code></sub></div>
