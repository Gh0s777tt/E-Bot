<!-- SYNC: v0.240.0 · #310 · 2026-06-19 — utrzymywane przez `pnpm docs:check` (NIE edytuj ręcznie wersji bez aktualizacji statusu) -->
<div align="center">
...
![i18n panelu](https://img.shields.io/badge/i18n_panelu-🔄_30%2F~40-FFB020?labelColor=0a0a0a)
...
- [x] **Zrobione 30/~40 stron**: `/welcome` `/modules` `/leaderboard` `/logging` `/audit` `/modmail` `/birthdays` `/suggestions` `/appearance` `/wishlist` `/notifications` `/live` `/tickets` `/security` `/levels` `/counters` `/responder` `/automations` `/scheduled` `/diagnostics` `/stats` `/profile` `/donations` `/economy` `/eco` `/library` `/integrations` `/gaming` `/creator` `/roles`
- [ ] **Pozostało ~8 stron**: `/ai` `/applications` `/commands` `/custom-commands` `/engagement` `/moderation` `/settings` `/setup`
...
| **i18n treści + UI** | `0.202`–`0.240` | Samouczek, asystent, „Jak to działa?" 37/37, web GameVault (+RTL), **UI panelu (w toku, 30/~40)** | 🔄 |
...
- [🔄] **i18n UI panelu** — 30/~40 stron *(patrz „Bieżący tor" u góry)*

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
<div align="center"><sub>Ostatnia aktualizacja: 2026‑06‑19 · v0.240.0 (#310) · powiązane: <a href="ROADMAP.md">ROADMAP</a> · <a href="../CHANGELOG.md">CHANGELOG</a> · weryfikacja sync: <code>pnpm docs:check</code></sub></div>
