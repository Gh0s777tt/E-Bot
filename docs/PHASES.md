...
- [x] **RTL — przyrost 1 (powłoka)**: `Sidebar`/`Shell`/`Topbar` na logiczne klasy Tailwind v4 (`start`/`end`, `ps`, `ms`, `border-e`) — rail i offset treści odbijają się w arabskim. ✅ **v0.255.0**
...
- [ ] **Pełne lustrzane RTL** — fundament `dir="rtl"` (v0.254.0) + powłoka Sidebar/Shell/Topbar (v0.255.0) gotowe; pozostaje zamiana fizycznych klas Tailwind (`ml-`/`pl-`/`left-`/`text-left`) na logiczne w pozostałych ~100 komponentach + `MobileNav` (drawer/translate)
